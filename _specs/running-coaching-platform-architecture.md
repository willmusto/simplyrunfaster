...
Per-athlete opt-in to be notified when a wearable brand's integration ships (see Section 6). A row's presence with `notify = 1` means the athlete wants a heads-up; disabling the Settings toggle deletes the row. MariaDB-safe (utf8); **MyISAM with no FK on the live DB** — the migration's declared InnoDB/FK does not apply (production is MyISAM throughout; see `sql/schema.sql`).
...
Audit log written by the 90-day data-retention cron (`scripts/cron_delete_expired_accounts.php`, see §25). One row per anonymized account. MariaDB-safe (utf8); MyISAM on the live DB (production is MyISAM throughout).
...
**Pull (watch → log).** Webhook `POST /webhook/intervals` (`public/webhook_intervals.php`, public/pre-auth, wired in `index.php`) on `ACTIVITY_UPLOADED` / `ACTIVITY_ANALYZED`, plus a 30-day backfill on connect. **Secret verification (2026-06-22 fix):** Intervals.icu delivers the shared secret **inside the payload body** (its separate Authorization-header field is left blank), so the handler verifies the secret **from the payload** — constant-time (`hash_equals`) matched against `INTERVALS_WEBHOOK_SECRET` wherever Intervals places it in the body; the `X-Intervals-Webhook-Secret` header is only a fallback. A mismatch returns **401**. **Every delivery is logged to `intervals_webhook_log` BEFORE the secret check** (status `received`, then updated to `processed`/`skipped`/`failed`), so even a rejected (401) delivery is visible (`status='failed'`, `error_message` "rejected: secret mismatch …" — the ENUM has no `rejected` member). *This corrected the original handler, which checked an auth header Intervals never sends and logged only after auth — every real delivery 401'd before the log insert, so inbound sync was silently dead with an empty log.* **Event shape (2026-06-23 fix):** Intervals batches events under a top-level `events[]` array; each event carries its own `type` / `athlete_id`, and an activity event nests the activity object at `events[].activity` with its id at `events[].activity.id`. The handler iterates `events[]` and dispatches each. For each `ACTIVITY_UPLOADED` / `ACTIVITY_ANALYZED`, `IntervalsService::pullActivity()` does `GET …/activities/{id}` — which returns a **single-element list** (`[{…}]`) that it unwraps — and maps `type`/`distance`/`moving_time`/`start_date_local`/`average_heartrate`/`max_heartrate`. Run activities map to `completed_workouts` (`source='intervals'`), idempotent on the `(source, external_activity_id)` unique key, then the post-completion pipeline runs (compliance, training-load recompute, RPE prompt, return-to-running progression). Unmatched activities insert as unplanned and raise an `unmatched_activity` info flag (idempotent on `(athlete_id, intervals_activity_id)` so a re-delivered/re-analyzed run doesn't stack duplicates) — **except a run dated before the athlete's first-ever plan start** (`MIN(training_plans.plan_start_date)` across all statuses), which still imports + shows off-plan in the Log but raises **no** flag (a pre-plan run isn't meaningfully off-plan; a run in a gap between plans is ≥ the first start, so it stays in-plan-era and is flagged).

**Manual re-sync (push, plan → watch).** `IntervalsService::repushAllVisible($userId, $db)` re-pushes every visible, non-cancelled workout in the active plan (upsert, each logged to `intervals_push_log`). Exposed two ways: coaches via `POST /app/integrations/intervals/repush` (their own athletes); athletes via a **"Sync workouts now"** button in Settings → `POST /app/integrations/intervals/sync-athlete`.

**Manual re-sync (pull, watch → log).** `IntervalsService::backfillDetailed($userId, $days, $db)` re-runs the direct activity pull for a window (`days` ∈ {30, 60, 90}), reusing the same per-id `pullActivity()` as the on-connect backfill (so it inherits the list-unwrap + nested mapping) and returning a summary `{fetched, imported_new, skipped, errors, partial}` (idempotent; already-imported activities are skipped, not re-pulled; a wall-clock budget returns partial results rather than hitting the NFSN web timeout). Exposed to coaches/assistants on the athlete **Log tab** via a "Re-sync activities" control → `POST /app/integrations/intervals/backfill` (auth mirrors the Log tab: `CoachAssignments::canAccess`). This is also the recovery path for any run a webhook misses.

**Routes / config.** `GET /app/integrations/intervals/connect` · `GET …/callback` · `POST …/disconnect` · `POST …/repush` · `POST …/backfill` (coach Log-tab re-sync) · `POST …/sync-athlete` · public `GET …/guide` (setup walkthrough, `views/static/intervals_setup.php`). Secrets `INTERVALS_CLIENT_ID` / `INTERVALS_CLIENT_SECRET` / `INTERVALS_WEBHOOK_SECRET` / `INTERVALS_REDIRECT_URI` are empty placeholders in `config/config.php`; real values live only in `config/config.local.php`.
...
**Unified sub-view tab strip (2026-06-18):** The athlete view's mixed entry points are consolidated into one shared chrome — a back link, the athlete header (name, phase · week · race meta, on-track dot), and a horizontal sub-nav tab strip **Plan · Log · Messages · Profile · Flags** — rendered by `views/coach/partials/athlete_chrome.php` (+ `athlete_tabs.php`) at the top of every athlete sub-page, so header and strip are identical everywhere and the active tab reflects the current page. The meta + open-flag count come from the pure `CoachController::athleteChromeData()` (reusing existing getters — no new logic); each sub-page controller sets `$chrome` + `$chromeActive`. The **badge count and the on-track dot are single-sourced** from `CoachController::openFlagsForAthlete()` — the athlete's open flags across **both** tables (`engine_flags` + `coaching_intelligence_flags`), the same set the Flags tab's Open section renders — so badge, dot, and tab can't drift. The badge counts every open flag (no carve-outs); the dot's top severity is critical→red / warning→amber / else green (intel `opportunity`/`info` read green). This replaced a pre-CIL artifact where the badge counted open engine flags only (an open intel flag with no open engine flag showed on the tab while the badge read 0). Each tab links to its existing route (Plan = macro plan, Log = `athleteLog`, Messages = `coachMessages`, Profile = the coach edit page); the Messages full-screen chat hosts the strip inside its own header. The **Flags** tab (`GET /coach/athlete/:id/flags` → `athleteFlagsPage`) is the athlete's **full flag record** (open + resolved history) — a distinct read-only "review everything" surface, not a mirror of Plan's actionable OPEN ALERTS. The pure `CoachController::athleteFlagRecord()` aggregates **both** `engine_flags` and `coaching_intelligence_flags` across all statuses (last 90 days, cap 50 resolved), normalized and split into **Open** (current flags, newest first) and **Resolved** (history, most-recently-resolved first). Each resolved card carries a resolution stamp — Dismissed / Acted on / Auto-resolved, when, by whom (coach name; engine flags only), and the dismissal reason when recorded; a `superseded` intel flag (the predictive→reactive handoff) reads as **Auto-resolved**, never a coach action. No dismiss/act controls on the tab (acting stays on the Plan view / Intelligence page). The tab-strip Flags badge counts the **open flags across both tables** (the Open section's count, single-sourced via `openFlagsForAthlete()`), not the resolved history. Active-tab treatment: accent text (#5DCAA5 dark / #0F6E56 light), weight 500, #1D9E75 underline; inactive uses `--text-secondary`. The redundant "Training log →" header link, sidebar "Edit profile" button, and Messages "Profile →" link were removed (one way to reach each sub-view). Pure UI — no migration, no engine calls, no change to any sub-page's data/scope.
...
- **Matched** rows (`planned_workout_id` set, `LEFT JOIN planned_workouts`) also show the planned title + target duration + `compliance_score` and link to the existing workout thread (`/app/coach/workout/{planned_workout_id}/thread`); a 💬 indicator counts `session_notes`. **Unplanned** rows (no `planned_workout_id`, e.g. an off-plan Intervals pull) are marked "Off-plan" and expand inline to read-only actuals **plus a two-sided comment thread keyed on `completed_workout_id`** (there is no plan side, so no planned-vs-actual). Both parties share one thread, same `session_notes` store as matched sessions: the coach posts from the Log card → `POST /app/coach/athlete/:id/session-note` (`completed_workout_id`, optional whitelisted `return_to`, `CoachController::coachSessionNoteSave`); the athlete sees and replies on the existing session view (`/app/log/:id` → `views/athlete/session.php`, which already keys on `completed_workout_id` and renders the no-plan case) → `POST /app/log/note`. The 💬 `note_count` counts `session_notes` by `completed_workout_id` OR `planned_workout_id`, so off-plan threads get the indicator too; each side notifies the other (`coach_session_comment` / `athlete_session_note`).
...
### 9a. Engine-critical data requirements & completeness gate *(2026-06-23)*

The engine executes faithfully on whatever data it is given, so missing inputs produce wrong plans rather than errors (the Bekah case: a NULL `longest_recent_run_mins` defaulted to 0 and — before the classifier floor — collapsed a capable runner to `insufficient`, yielding a run/walk plan). Hardening, in two layers:

- **Engine-critical fields** (`ProfileForm::ENGINE_CRITICAL` + `missingCritical()`): `current_weekly_minutes`, `training_days_per_week`, `longest_recent_run_mins`, and `goal_race_distance` (the last required **only** on the race-cycle path; N/A for development / maintenance / return-to-running).
- **Required-field enforcement (Stage A3):** captured-and-required on **every** path that feeds generation. Self-serve / invite **onboarding** requires them at steps 1/2/4. The **coach Edit Profile** and **athlete Training Settings** forms (both `ProfileForm::sanitize` → `validateSubmission`) now **block the save** when a critical field is missing, and surface non-blocking soft-sanity notes ("double-check…") alongside a successful save. Previously the coach/edit path left all four optional — the NULL source for coach-created / incomplete-invite athletes.
- **Generation completeness gate (Stage B, `CoachController::generatePlan`):** the safety net for athletes who never completed full onboarding. Before generating, if any engine-critical field is missing it **warns-and-allows** — bouncing back with a banner naming exactly what's absent and the default the engine would substitute ("…the engine will assume a 5K goal; estimate the long run…"), with Edit Profile one click away. A second click (`ack_missing=1`) proceeds anyway. A silent NULL-driven default can no longer ship unnoticed.

No new columns — all inputs derive into the existing canonical columns. The classifier itself (`PlanGenerator::classifyAthlete`) independently excludes missing axes and floors capable athletes (see engine spec §5), so even an acknowledged-incomplete generation is non-catastrophic.
...
**`athlete_behavior_log`** — daily behavior metrics; **90-day rolling retention** pruned by the daily cleanup cron (`cron_delete_expired_accounts.php`). `metric_type` ENUM (rpe_vs_target, completion_rate, engagement_score — the two never-produced members `easy_pace_drift`/`response_time` were removed in migration_034), `metric_value` FLOAT, `metric_context` LONGTEXT, `plan_week`, `phase`.
