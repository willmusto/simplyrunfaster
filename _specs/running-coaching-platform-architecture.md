...
- **Per-user preference.** `users.timezone` holds an IANA identifier (default `America/New_York`, migration_008). Athletes set their own on the Training Settings page; a coach can override an athlete's on the athlete edit page (COACH CONTROLS); a coach sets their own display timezone on coach Settings.
