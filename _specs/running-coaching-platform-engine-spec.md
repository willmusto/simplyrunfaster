...
16. **Coach macro-view shows a 13th partial week and doubled "Cutback" labels for mid-week-start plans** *(display-layer, not a generation bug)* — Observed on Liam after item 15: the coach individual-athlete macro view renders **13 weeks** with **back-to-back "Cutback" labels at calendar weeks 4&5 and 8&9**. Investigated and confirmed to be a **Mon–Sun calendar-week grouping artifact**, not a generation issue:
    - The **generation is correct**: 12 internal code-weeks (7-day blocks from plan start), single cutbacks every 4th week at cw4/cw8/cw12 (verified per-7-day volume trace: 130, 140, 151, **121**, 163, 176, 190, **152**, 203, 220, 238, **190**). No back-to-back cutback code-weeks; §6's "every 4th or 5th week" cadence holds.
    - The view groups workouts into **Mon–Sun calendar weeks** (the documented partial-first/last-week behavior, §15 / Architecture). `plan_start_date = +1 day`; when that is mid-week (Liam's regeneration started **Tuesday** Jun 16), a 12-code-week / 84-day plan spans **13** Mon–Sun weeks — a partial first week plus a 13th week holding a single leftover workout.
    - The **doubled cutback labels** come from `views/coach/athlete_view.php`: each calendar week is labeled "Cutback" if **any** internal code-week overlapping it is a cutback. A cutback code-week that straddles the mid-week boundary overlaps **two** Mon–Sun weeks (cw4 = Tue Jul 7 – Mon Jul 13 falls in cal weeks 4 & 5; cw8 likewise in 8 & 9), so both get the label.
    - **Plan-length change to 13 is unrelated to items 1–4.** Plan length is still 12 code-weeks and `plan_start` is unchanged; the calendar-week count is purely a function of the generation weekday. (The macro view already reported 13 weeks for Liam before items 1–4.) Items 1–4 changed per-week day counts and the volume trajectory, not plan length or start date.
    - **Fix direction (deferred):** either (a) align `plan_start_date` to the athlete's week start (e.g. the upcoming Monday, or the long-run day) so code-weeks coincide with calendar weeks and partial weeks/doubled labels disappear; or (b) attribute each calendar week to a single dominant code-week (e.g. the one containing that week's long run or the majority of its days) for phase/cutback labeling, instead of flagging on any overlap. Option (a) also removes the near-empty 13th week. Engine generation needs no change either way.
