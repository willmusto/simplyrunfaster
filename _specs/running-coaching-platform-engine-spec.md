...
**Rolling-window maintenance.** After updating `rtr_current_stage`, `regenerateReturnToRunningWindow` rebuilds the *future* portion of the plan: it deletes uncompleted, **non-coach-locked** planned workouts dated after the just-completed session, then regenerates from the day after it — the **next** eligible run day (every-other-day cadence anchored on the completed session, never on `must_off_days`) becomes a single `run_walk_intervals` session re-staged to the new stage, and every other day to the `RTR_WINDOW_DAYS` (10-day) horizon is cross-training/rest with the coach-drill note. **Exactly one pending run/walk session is kept ahead at any time** — when the athlete completes it the stage updates again and the window is rebuilt from there. Coach-locked future sessions are preserved (the coach's manual hold/advance/regress per §24 wins), and their dates are skipped so the regeneration never double-books a day. `plan_end_date` is extended (never shrunk) to the new window end, and days inside the live `[today, horizon]` window are made `visible_to_athlete = 1` immediately — the same horizon the daily `cron_update_visibility` job and plan approval open — so the athlete sees the next session without waiting for the nightly cron. On a clean stage-10 finish the window is filled with cross/rest only (no further run) until the coach acts. `validateGeneratedDisplays()` runs after every patch, exactly as in `generate()`.
