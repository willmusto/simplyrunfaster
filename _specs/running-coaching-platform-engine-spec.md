...
estimate and sets `pace_zones_source = 'race_result'` (verified). Editing the most-recent-race
result on the coach/athlete profile form derives the same verified `race_result` zones at
save time (`ProfileForm::maybeDeriveRaceZones` → `fromRace`; see architecture §31), taking
precedence over the easy-pace estimate. The easy-pace estimate pathway never overwrites
verified or manual zones. Implemented in `src/Engine/PaceZones.php`.
...
**Race-time plausibility floor (input validation).** A race result only derives zones
when its implied pace is physically plausible — between **150 and 1320 sec/mile**
(≈2:30–22:00/mile), enforced by `PaceZones::isPlausibleRaceTime()`. `fromRace()` returns
`null` for an out-of-range time, so an impossible entry (e.g. a marathon mis-entered as
"4:02" → parsed `mm:ss` = 242 s = a ~4-minute marathon) derives **no** zones and the
athlete falls back to the effort-only / no-zones path rather than garbage single-digit-
second paces. The identical check runs at **every** race-time entry door — onboarding
(`OnboardingController::saveStep2`) and the coach/athlete profile form
(`ProfileForm::validateSubmission`) — rejecting an implausible time with a clear message
before it is stored. Race times are entered `h:mm:ss` / `mm:ss` and stored as canonical
**seconds** via the shared `ProfileForm::parseRaceTime()` parser (never a raw-seconds
field). Distances the engine can't project (ultras) skip the floor — there's no pace
basis to judge them and `fromRace()` already declines to derive zones for them.
...
- **Instructions.** `PlanGenerator::wrapWithWarmupCooldown()` now prepends a warmup sentence and appends a cooldown sentence to all 10 wrapped structured quality archetypes, producing `[Warmup]. [Main-set]. [Cooldown]. [Pace citation if any]`. The **six** strides-warmup archetypes (`equal_distance_repeats`, `short_speed_repeats`, `sustained_hill_repeats`, `hill_sprints`, `plyometric_hill_circuits`, `hill_sprint_ladder` — the `WARMUP_WITH_STRIDES_ARCHETYPES` constant) read "Warm up with N minutes of easy running, finishing with 4 × 15-second strides with full recovery between each."; the **four** no-strides archetypes (`tempo_intervals`, `high_volume_time_intervals`, `structured_fartlek_ladder`, `mixed_distance_repeats` — `WARMUP_NO_STRIDES_ARCHETYPES`) read "Warm up with N minutes of easy running." All append "Cool down with N minutes of easy running." The wrap runs after `normalizeInstructionText()` and **before** `appendPaceCitation()`. **`run_walk_intervals`, `standalone_strides`, and `continuous_progression_tempo` are excluded** — their own `description_template`s already carry the warmup/cooldown language, so wrapping would double it. (`continuous_progression_tempo`'s template self-supplies "Warm up with {{warmup_minutes}} minutes of easy running." / "Cool down with {{cooldown_minutes}} minutes of easy running." around `{{progression_instruction}}`; see §18.8. The descriptions rewrite once dropped that wrapper from its template while it stayed on the exclusion list, leaving no warm/cool text at all and tripping the validator below — restored June 2026, commit 1453858.)
...
> **Foundation scope vs. §24 Time Off Classification.** The current pathway always starts at **stage 1** and prescribes **one session per stage** (10 sessions → stage 10). The §24 *Time Off Classification* table (break length → a higher starting stage) and its "hold each stage for 2 sessions for an extended break" are **not yet wired** — every athlete begins at stage 1 and advances one stage per clean session. Wiring the break-tier starting stage / per-stage hold is a follow-on.
...
### 18.12 Concrete prescription lead line + shared recovery resolver (2026-06-24)

Athletes read the **description** (on the phone / in-app), not only the watch steps, but the rewritten prose omitted the hard numbers — a tempo read "A tempo session broken into sustained intervals…" but never said "4 × 14 min" or the recovery. `PlanGenerator::prependPrescriptionLead()` now prepends a concrete prescription line, then the existing prose unchanged:

> **"N × [length] at [effort], [recovery] between reps."** + prose

It runs **last** in the instruction pipeline (after `appendPaceCitation()`), so the final order is `[Lead]. [Warmup]. [Main-set]. [Cooldown]. [Pace citation]`. Scope: the **5 uniform-rep quality archetypes** — `tempo_intervals` ("4 × 14 min at comfortably hard (tempo) effort, ~2 min 45 sec easy jog between reps"), `sustained_hill_repeats` ("8 × 90 sec uphill … jog back down to the start between reps"), `equal_distance_repeats`, `short_speed_repeats`, `high_volume_time_intervals`. `mixed_distance_repeats` and `structured_fartlek_ladder` already lead with their full sequence in their own templates (§18.8) and are **not** re-led; `hill_sprints` keeps its count in the **title** (`{{sprint_count}} × …`). The effort phrase uses **words only**; the numeric pace stays in the §18.9 citation, so pace is never double-cited.

**Recovery comes from a shared resolver, so the description's recovery and the watch's recovery step are guaranteed to agree.** The recovery math that was private to `IntervalsService` was extracted into the engine classes both sides call (watch output byte-for-byte unchanged, just sourced from the shared resolver):

- `RecoveryModel::resolveSeconds($model, $workSeconds, $explicitOverride)` — an explicit `recovery_duration_seconds` override wins; otherwise the `recovery_model` slug is applied to the work duration (`modelSeconds`: ratio or `fixed_seconds`), then rounded (`roundSeconds`). Per archetype: `tempo_intervals` `threshold_standard` (0.20× rep duration); `equal_distance_repeats`/`mixed_distance_repeats` `vo2_standard` (≈1:1); `short_speed_repeats` `speed_standard` (fixed ~90 s); `high_volume_time_intervals` the **stored** `recovery_duration_seconds`; `sustained_hill_repeats` the jog-back-down cue (no number).
- `PaceZones::estimateRepSeconds($meters, $effort, $zones)` + `PaceZones::paceKeyForEffort()` — the rep-duration estimate for distance reps (from visible pace zones, ~6:30/mi fallback when hidden), so the description's distance-rep recovery matches the watch's.

Recovery is therefore **computed at render, not stored** (the exception is `high_volume_time_intervals`, which persists `recovery_duration_seconds`). The Structure tab's editable "Recovery" field maps to that same `recovery_duration_seconds` override (usually blank for generated tempo/hills/equal/short/mixed). Commit d7dfe4d.

### 18.13 Structure-preserving description re-render + backfill (2026-06-24)

`athlete_instructions` is **stored at generation** (the app reads the stored column; it is not re-rendered per view), so template, copy, and pace-zone fixes are **tomorrow-and-beyond by date** — existing rows keep their stored text until they regenerate or are explicitly re-rendered.

`PlanGenerator::recomposeDescriptionOnly($id)` re-renders a workout's **description only** from its **stored `archetype_params` verbatim** + the athlete's **current** pace zones, with **no change to the workout**. It runs `addDerivedParams()` in `$manual` mode (no ranging/cap/snap/re-roll) and only the display/render tail, so the rebuilt `structure` is byte-identical to the stored one (the caller asserts this and persists **TEXT columns only**). It differs from `composeStructuredEdit()`, which re-derives `rep_distance_miles` from the athlete's current pace to support edits and so is not structure-preserving. Scope: the 6 `STRUCTURED_EDIT_CODES` + `continuous_progression_tempo` + **standard** `structured_fartlek_ladder` — it **refuses the `diminishing_descending` fartlek variant** (its description is an `addDerivedParams` code override that only fires on fresh generation and cannot be reproduced on re-render); `fast_finish_long` is excluded for the same reason.

**The descriptions-rewrite regression class.** The June 2026 archetype-descriptions refresh (da156ec, source `database/seeds/archetype_descriptions.json`) replaced templates with new prose. For archetypes whose description renders **straight from the DB template with no code override**, the rewrite could silently drop an instance-specific token while the engine still computed it. Two cases occurred:

- `continuous_progression_tempo` — dropped the warm/cool wrapper **and** `{{progression_instruction}}` while staying on the `wrapWithWarmupCooldown` exclusion list (§18.7) → no warm/cool text → **caught** by the §18.8 validator (`missing_warmup_cooldown_text`, `display_generation_incomplete`). Restored June 2026 (commit 1453858).
- `structured_fartlek_ladder` (standard variants) — dropped `{{round_count}}`/`{{fartlek_ladder_sequence}}` (the actual ladder, e.g. "3 × 90-60-30 sec"); **not** caught (warm/cool still came from the wrapper, a digit from the warmup sentence) — a pure silent degradation. Restored June 2026 (commit 1252cd7). The diminishing variant was unaffected (its code override still renders the sequence).

Both were restored at the template/data layer (idempotent migrations `migrate_{cpt,fartlek}_description_fix.php` + the seeds JSON, now the source of truth, so re-running the big descriptions migration won't re-break them), then the few already-degraded future rows were re-rendered via `recomposeDescriptionOnly` (text only, structure asserted identical). A token-drop **sweep** confirmed the other rewrite drops (the 5 uniform-rep archetypes + `hill_sprints`) are covered by their title templates and/or the §18.12 lead line, so they are not silently degraded. The same `recomposeDescriptionOnly` backfill also gave existing future quality descriptions the §18.12 lead line and refreshed their pace citations from current zones; for archetypes that can't be safely re-rendered, a **surgical pace-range swap** corrects only the wrong `M:SS/mile` token (used to fix an athlete whose `pace_zones` were corrected after generation, e.g. stale `0:08/mile`). The watch is unaffected throughout (it renders from the unchanged `structure`). Commits 66fda41, d7dfe4d, 1453858, 1252cd7.
...
6. **Beginner-specific archetype gating** — ✅ **Resolved.** Two beginner archetypes exist as real entries: `run_walk_intervals` (10 deterministic stages) and `standalone_strides`, both `min_classification: insufficient` and insufficient-only weighted (see §18.10). `return_to_running` has a dedicated generation pathway (`generateReturnToRunning`) that no longer falls back to `continuous_easy` — it **pre-generates the full progression upfront**: all 10 run/walk sessions at their expected stage (session N at stage N, stage 10 the first continuous run) on an every-other-day cadence (respecting `must_off_days`, ~20 days), with equipment-matched cross-training or a generic gentle rest day on off days (no rehab/coach-drill note). The initial 10-day rolling window is opened (`visible_to_athlete=1`); sessions beyond it stay coach-only (`visible_to_athlete=0`) until the window reaches them, and `plan_end_date` spans the whole progression. `training_plans.rtr_current_stage` (migration_006) tracks the athlete's current stage, initialised to 1. Development-plan insufficient-base athletes draw `run_walk_intervals` (fixed stage 1) and `standalone_strides` for their quality/easy slots instead of `continuous_easy`-only. **The adaptive per-session stage-progression mechanic is now built** (`PlanGenerator::onRunWalkCompletion`): on each logged run/walk completion the engine reads the modified 5-option RPE response and advances (clean, stage < 10), holds + raises a `plan_rebuild_needed` transition flag (clean stage-10 first continuous run), or auto-regresses one stage with floor 1 + raises the existing `return_to_running_discomfort` flag (discomfort). It then re-stages the **next pending** run/walk session in place to the new stage (the rest of the pre-generated progression is left intact; a safety-net session is appended only if discomfort regressions exhaust the pre-generated set). Full mechanic documented in §18.11. Verified by regenerating an RTR athlete's plan (staged progression 1..10 with correct rep counts, the visibility split, and in-place re-staging of the next session on a clean completion) with `validateGeneratedDisplays()` clean throughout.
...
13. **`validateGeneratedDisplays()` does not check warmup/cooldown description completeness** — ✅ **Resolved.** The post-generation validator now carries a third condition: for any workout whose `resolved_params` has `warmup_minutes > 0` or `cooldown_minutes > 0`, the combined display text (title + summary + instructions) must contain both "warm" and "cool" (case-insensitive), else a `missing_warmup_cooldown_text` reason is recorded and the `display_generation_incomplete` flag is raised. This backstops the warmup/cooldown wrapping itself: the 10 wrapped structured quality archetypes carry only main-set language in their `description_template`, and `PlanGenerator::wrapWithWarmupCooldown()` now prepends a warmup sentence ("Warm up with N minutes of easy running" — plus "finishing with 4 × 15-second strides…" for the six strides-warmup archetypes) and appends a cooldown sentence ("Cool down with N minutes of easy running"), inserted after `normalizeInstructionText()` and **before** `appendPaceCitation()` so the cooldown precedes any pace citation. The self-contained `run_walk_intervals`, `standalone_strides`, and `continuous_progression_tempo` sessions already include warmup/cooldown language in their own templates and are excluded from wrapping; run/walk stage 10 (the first continuous run) carries `warmup_minutes = cooldown_minutes = 0`, so the validator does not require the language there. This validator is exactly what later caught a regression where `continuous_progression_tempo`'s self-supplied wrapper was dropped by a template rewrite (it tripped `missing_warmup_cooldown_text`; restored June 2026). See §18.7. A sibling silent regression that this validator did **not** catch — `structured_fartlek_ladder` (standard variants) losing its `{{round_count}}`/`{{fartlek_ladder_sequence}}` ladder while warm/cool still came from the wrapper — is documented in §18.8.
...
17. **Classification persistence, 7-day rest days, Saturday rest bias, and missing-race-date guard** — ✅ **Resolved** (this session). Four engine fixes, documented in **§5** / **§6**:
    ...
    - **Dateless race cycle:** `generateRaceCycle` previously fell back to a development plan when `goal_race_date` was NULL. It now raises a **critical flag and returns early** (onboarding requires the date for race-cycle goals; §6 "Missing Goal Race Date"). The flag reuses the valid `plan_rebuild_needed` type with `details.reason = 'missing_goal_race_date'`: a literal `missing_goal_race_date` was **never an `engine_flags.flag_type` ENUM member**, so on MyISAM it was silently coerced to `''` and the critical flag disappeared from every coach view — fixed by reusing the valid member (one historical `''` row backfilled). See `_specs/archive/db_debt_audit.md` §1.
...
19. **Coaching Intelligence decision resolver (generation touchpoint)** — ✅ **Resolved.** Coach-authored rules can adjust the quality-archetype selection at generation time. `PlanGenerator::generate` loads the active decisions for the athlete's coaches once (`CoachingDecisions::loadActiveForAthlete`); inside `insertWeekWorkouts`, `applyCoachingDecisions` matches each rule's `trigger_json` against the per-week context (goal_distance / phase / classification / plan_type — absent keys are wildcards) and applies its `action_json` to the weighted draw: `exclude_archetypes` (removed from the pool), `weight_multipliers` (multiplied into selection weights), `force_archetype` (dominant weight), `max_quality_per_week` (extra quality slots fall back to `continuous_easy`). Conflicts on the same archetype resolve to the **higher (more recent) decision id**, logged to `training_plans.coach_generation_notes`. This is **additive** — it only reweights/excludes within the existing pool; the volume/phase/cutback/continuity math is untouched, and an athlete with no matching rules generates identically to before. Full layer (capture → propose → approve → share, predictive flags, multi-coach) documented in **Architecture §32** (Coaching Intelligence Layer, Phases 1–4; migrations 027–030).

20. **Regen carry-over of athlete-exposed weeks** — ✅ **Resolved.** A regeneration now **preserves whole weeks the athlete has already seen** by default rather than wiping and rebuilding. **The generation math is unchanged:** `generate()` still builds the full span exactly as before (volume progression, cutback cadence, phase/taper, lead-in/code-week anchoring, cross-cycle continuity). Around it, `capturePreservation()` records the prior active plan's exposed whole weeks (any Mon–Sun week with a `visible_to_athlete` row) plus any `coach_locked` row; carried signatures are merged into the anti-repeat history so the fresh remainder never duplicates them within the hard-block window; then `applyPreservation()` deletes the freshly-generated rows on the carried dates and **moves** the prior rows into the new plan (same id → the `srf_{id}` Intervals event survives), so exposed weeks are byte-identical and the fresh remainder begins at a clean Monday. A required full-wipe checkbox forces the legacy archive-all + rebuild. Migration_031; full mechanic + lifecycle in **Architecture §33**.
