...
10. **`short_speed_repeats` variant display uniformity** *(promoted to active — now the primary variety constraint)* — Variants `speed_300s`, `economy_200s`, `speed_endurance_400s`, `broken_speed_set`, and `speed_endurance_600s` currently all resolve to the same display title format (e.g. `7 × 200m`) for a workable athlete. **Status:** With quality-session duration scaling in place (§6), the low-volume quality pool is no longer starved — `tempo_intervals`, `sustained_hill_repeats`, `equal_distance_repeats`, and `continuous_progression_tempo` now fill mid/high-volume weeks. The remaining variety limit traces almost entirely to this item: because all five `short_speed_repeats` variants resolve to identical `7 × 200m` params, each use 28-day-hard-blocks the other four (signatures differ only by variant name), so `short_speed_repeats` can appear at most once per ~4 weeks regardless of budget. Resolving this — giving each variant a genuinely distinct rep distance/structure (e.g. `economy_200s` → 200m, `speed_endurance_400s` → 400m, `speed_endurance_600s` → 600m) so their signatures and displays differ — is the highest-leverage remaining variety fix. Confirm whether this is correct: variant names likely describe effort targets or training emphasis rather than different rep distances — all variants may prescribe 200m reps but at different intensities or with different recovery structures, which would make uniform titles correct. If variants are instead meant to prescribe different rep distances, the `rep_distance_meters` parameter or per-variant `display.title_template` overrides are the fix. Needs a quick review of each variant's JSON definition in the `workout_archetypes` table. **Interaction with item 11:** For low-volume athletes where `short_speed_repeats` fills most or all quality slots, whether variants prescribe different rep distances or the same distance with different effort targets becomes more consequential — uniform display titles across variants make weeks of `short_speed_repeats` visually indistinguishable in the plan view even if the underlying training stimulus genuinely varies. Review items 10 and 11 together.
